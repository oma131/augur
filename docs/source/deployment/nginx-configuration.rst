...
Once nginx is configured, issue these commands to make sure everything is loaded and configured correctly:
...
1. `augur.config.json` server block **must** be set like this:
...
2.   Compile augur (this wires the host and port into the frontend so people pulling the web pages of Augur, in the `frontend/` subdirectory are referring to the correct endpoints for this instance.): ``make rebuild``
...
This file will be located in the ``/etc/nginx/sites-enabled`` directory on most linux distributions.  Mac OS X keeps these files in ``/usr/local/etc/nginx/sites-enabled`` **Note that Augur's backend server must be running**
